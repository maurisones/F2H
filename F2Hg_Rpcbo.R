...
library("utiml")  # install from CRAN or https://github.com/rivolli/utiml devtools::install_github("rivolli/utiml")
library("mldr")
library("RWeka")
library("foreach")
library("doParallel")
...
library("digest")
library("BBmisc")
library("Rpcbo")  # https://github.com/maurisones/Rpcbo devtools::install_github("maurisones/Rpcbo")

dsname <- "birds"
#dsname <- "yeast"
dsdir <- "/Users/mauriferrandin/Downloads/F2H/"
dsdir <- "C:/Users/Mauri Ferrandin/Downloads/F2H/"
clusExe <- "java -Xmx3g -jar /Users/mauriferrandin/Downloads/Clus/Clus.jar"
clusExe <- "java -Xmx3g -jar \"C:/Users/Mauri Ferrandin/Downloads/MyClus.jar\""
...
if (length(args) != 0) {
  ...
  dsdir <- args[2]
  clusExe  <- args[3]
  minSupportConcetps <- as.numeric(args[4])
  threads <- as.numeric(args[5])
  fold <- as.numeric(args[6])
  clusWType <- args[7]
  clusWParam <- as.numeric(args[8])
  clusOptimizeErrorMeasure <- args[9]
  ensemble = as.numeric(args[10])
...
dir <- paste(dsdir, dsname, sep = "")
setwd(dir)

train_file <- paste(dsname, "_train_", fold, sep = "")
test_file <- paste(dsname, "_test_", fold,  sep = "")
valid_file <- paste(dsname, "_valid_", fold,  sep = "")


# obtendo o arquivo de treinamento
...
times <- tic(times, "Fim leitura de arquivos")
...
dirf <- paste(dsdir, dsname, "/", fileid, "-fold", fold, sep = "")
...
compute_results_t1("tr")
compute_results_t1("va")
compute_results_t1("te")

compute_results_t2("tr")
compute_results_t2("va")
compute_results_t2("te")

compute_results_t3("tr")
compute_results_t3("va")
compute_results_t3("te")
...
infos[5] <- threads
infos[6] <- fold
infos[7] <- length(combs)
infos[8] <- length(edges)
infos[9] <- clusWType
infos[10] <- clusWParam
infos[11] <- clusOptimizeErrorMeasure
infos[12] <- ensemble
 
 
names(infos) <- c("Dataset", "Folder", "Min. Sup. PCBO", "Clus", "Threads", "Fold", "Number of concepts", "Number of edges","clusWType","clusWParam","clusOptimizeErrorMeasure", "ensemble")
write.csv(x = infos, file = paste(dsname, "-", fold, "-metadata.csv", sep = ""))
