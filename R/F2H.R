...
compute_nodeskohonen <- function(df){
  ...
  som_grid <- somgrid(xdim = (ncol(data_train_matrix)), ydim=(ncol(data_train_matrix)), topo="rectangular")
...
compute_nodeskmeans <- function(df){
  ...
  km <- kmeans(data_train_matrix, centers = (ncol(data_train_matrix)*5), iter.max = 10000)
  # #km$cluster
